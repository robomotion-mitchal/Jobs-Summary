function testDate(){
  ...
  var dtStr = dt[2] + "-" + dt[1];
  Logger.log(dtStr);
  //html += "<td width='80px'>" + new Date(rows[i][j]).toISOString().substring(5,10) + "</td>";
}
...
//       runAt = client.paradise.schedule.runAt;
...
//     scheduleDate.setHours(runAt.split(':')[0], runAt.split(':')[1], 0, 0);
